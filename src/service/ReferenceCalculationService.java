...
public class ReferenceCalculationService implements ReferenceCalculationServiceInterface {
    ...
    private MortgageReference reduceRateMortgageReference(RateAmounts rateAmounts, Rate previousRate) {
        if (rateAmounts.getOverpayment().getAmount().compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal referenceAmount = calculateResidualAmount(
                    rateAmounts, previousRate.getMortgageResidual().getAmount());
        ...
        return new MortgageReference(previousRate.getMortgageReference().getAmount(),
